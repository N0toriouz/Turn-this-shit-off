export async function onRequestGet(context) {
  ...
  // Bulk mode — totals for every album in one call, so the homepage album
  // grid doesn't fire a separate request per card.
  if (all) {
    try {
      const { results: albums } = await env.ALBUMS_DB.prepare(`SELECT * FROM albums`).all();
      ...
      for (const s of (songs || [])) {
        if (!totalsMap[s.album_id]) totalsMap[s.album_id] = zeroTotals();
        const t = totalsMap[s.album_id];
      ...
      const results = (albums || []).map(a => ({ ...a, ...(totalsMap[a.id] || zeroTotals()) }));
