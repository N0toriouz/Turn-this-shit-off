...
// subscribers. Album art is a plain URL, same fix applied here as the
// pre-release function, for the same reason (avoid email size truncation).
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const { albumName, trackCount, ctaUrl1, ctaUrl2, albumArtUrl } = body;
    ...
    for (const sub of subscribers) {
      try {
        await sendLiveReleaseEmail(
          sub.email, albumName, trackCount, ctaUrl1, ctaUrl2, albumArtUrl, env.RESEND_API_KEY
        );
        sentCount++;
      } catch (e) {
        failedCount++;
...
async function sendLiveReleaseEmail(toEmail, albumName, trackCount, ctaUrl1, ctaUrl2, albumArtUrl, apiKey) {
  ...
  // ── Same reordering as pre-release: CTA → title → album art ──
  const html = `
  <div style="background:#0A0A0A; padding:40px 20px; font-family:Arial,sans-serif;">
    <div style="max-width:480px; margin:0 auto; background:#111111; border:1px solid #2A2A2A; border-radius:4px; padding:32px;">
      ...
      ${trackCount ? `<p style="color:#888888; font-size:13px; margin:0 0 20px; text-align:center;">${trackCount} tracks, available now.</p>` : ''}
